...
class Board {
    ...
    Board(Board board0) {
        ...
        _numJumpArr = (ArrayList<Integer>) board0._numJumpArr.clone();
    ...
    void clear() {
        ...
        for (int i = 0; i < EXTENDED_SIDE * EXTENDED_SIDE; i++) {
            if (i % 11 == 0 || i % 11 == 1
                    || i % 11 == 9 || i % 11 == 10
                    || i < EXTENDED_SIDE * 2 - 2
                    || i > _board.length - 2 * EXTENDED_SIDE + 1) {
    ...
    boolean legalMove(Move move) {
        ...
        PieceColor startColor = get(move.col0(), move.row0());
        PieceColor endColor = get(move.col1(), move.row1());
        if (startColor != _whoseMove) {
            return false;
        } else if (!get(move.fromIndex()).equals(whoseMove())) {
            return false;
        } else if (startColor.equals(EMPTY) || startColor.equals(BLOCKED)) {
            return false;
        } else if (!endColor.equals(EMPTY) || endColor.equals(BLOCKED)) {
            return false;
        } else if (Math.abs(move.col0() - move.col1()) > 2
                || Math.abs(move.row0() - move.row1()) > 2) {
            return false;
        }
        return true;
    }
    ...
    boolean canMove(PieceColor who) {
        ...
        for (int i = 2 * EXTENDED_SIDE + 2;
             i <= _board.length - 2 * EXTENDED_SIDE - 3; i++) {
            if (get(i).equals(who)) {
                for (int j = -2; j <= 2; j++) {
                    for (int k = -2; k <= 2; k++) {
                        if (get(neighbor(i, j, k)).equals(EMPTY)) {
                            return true;
                        }
                    }
                }
    ...
    void makeMove(Move move) {
        ...
        if (move.isPass()) {
            pass();
            _winner = getWinner();
            _numJumpArr.add(_numJumps);
            return;
        }
        _allMoves.add(move);
        startUndo();
        int endSq = index(move.col1(), move.row1());
        set(move.col1(), move.row1(), whoseMove());
        PieceColor opponent = _whoseMove.opposite();
        for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
                int neighbor = neighbor(endSq, i, j);
                if (get(neighbor).equals(opponent)) {
    ...
    void undo() {
        ...
        while (_undoSquares.peek() != null) {
            int changeSq = _undoSquares.pop();
            PieceColor oldCol = _undoPieces.pop();
            PieceColor lastColor = _whoseMove.opposite();
            if ((oldCol.equals(RED) && get(changeSq).equals(BLUE))
                    || (oldCol.equals(BLUE) && get(changeSq).equals(RED))) {
                incrPieces(oldCol, 1);
                incrPieces(oldCol.opposite(), -1);
            } else if (oldCol.equals(EMPTY)) {
                incrPieces(lastColor, -1);
            } else if (get(changeSq).equals(EMPTY)) {
                incrPieces(lastColor, 1);
            }
            unrecordedSet(changeSq, oldCol);
        }
        if (_numJumpArr.size() == 1) {
            _numJumps = 0;
        } else {
            _numJumpArr.remove(_numJumpArr.size() - 1);
            _numJumps = _numJumpArr.get(_numJumpArr.size() - 1);
        }
        _whoseMove = _whoseMove.opposite();
    ...
    boolean legalBlock(char c, char r) {
        char cRefl = (char) ('d' - (c - 'd'));
        char rRefl = (char) ('4' - (r - '4'));
        if (_allMoves.size() != 0) {
            return false;
        }
        if (!(get(c, r).equals(EMPTY) && get(cRefl, r).equals(EMPTY)
                && get(cRefl, rRefl).equals(EMPTY)
                && get(c, rRefl).equals(EMPTY))) {
            return false;
        } else if (c < 'a' || c > 'g' || r < '1' || r > '7') {
            return false;
        }
        return true;
    }
    ...
    void setBlock(char c, char r) {
        ...
        if (get(cRefl, r) != BLOCKED) {
            unrecordedSet(cRefl, r, BLOCKED);
            _totalOpen -= 1;
        }
        if (get(cRefl, rRefl) != BLOCKED) {
            unrecordedSet(cRefl, rRefl, BLOCKED);
            _totalOpen -= 1;
        }
        if (get(c, rRefl) != BLOCKED) {
            unrecordedSet(c, rRefl, BLOCKED);
            _totalOpen -= 1;
        }
    ...
    public boolean equals(Object obj) {
        ...
        if (!_allMoves.equals(((Board) obj)._allMoves)) {
            return false;
        } else if (_numJumps != ((Board) obj).numJumps()) {
            return false;
        } else if (!_whoseMove.equals(((Board) obj)._whoseMove)) {
            return false;
        } else if (_totalOpen != ((Board) obj).totalOpen()) {
            return false;
        } else if (!_numPieces.equals(((Board) obj)._numPieces)) {
            return false;
        } else if (!_winner.equals(((Board) obj)._winner)) {
            return false;
        } else if (!Arrays.equals(_board, other._board)) {
            return false;
        } else if (!_undoSquares.equals(((Board) obj)._undoSquares)) {
            return false;
        } else if (!_undoPieces.equals(((Board) obj)._undoPieces)) {
            return false;
        } else if (!_notifier.equals(((Board) obj)._notifier)) {
            return false;
        }
        return true;
    }
    ...
    /** Store the history of _numJumps in an ArrayList. */
    private ArrayList<Integer> _numJumpArr;
}
